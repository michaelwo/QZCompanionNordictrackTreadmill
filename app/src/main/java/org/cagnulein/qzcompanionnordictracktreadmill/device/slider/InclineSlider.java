...
public class InclineSlider extends Slider {
    ...
    public void handle(double pct, Device device) {
        ...
        device.logger.log("QZ:Dispatch", "requestIncline: " + pct + " quantized=" + quantized + " last=" + last);
        if (last == null || quantized != last) {
            moveTo(quantized, device);
            device.logger.log("QZ:Dispatch", "applyIncline: " + quantized);
        } else {
            device.logger.log("QZ:Dispatch", "de-dup: skipping incline " + pct + " (quantized=" + quantized + " already at " + last + ")");
        }
    }
