...
| Failure Resilience | 3 | UDP listener loop catches per-packet exceptions and continues (no permanent thread death); MonoStdoutMetricReader stream errors caught in-loop (reader thread survives); performSwipe logs Log.e on null instance and dispatchGesture=false; CrashHandler logs QZ:Crash + saves to CrashPrefs before kill |
